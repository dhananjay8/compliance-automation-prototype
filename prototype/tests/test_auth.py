...
def _request(tenant_id: str | None = None):
    return SimpleNamespace(path_params={"tenant_id": tenant_id} if tenant_id else {})
...
def test_require_admin_role_mock():
    auth.require_admin_role(x_user_id="u1", x_user_role="admin")


def test_require_admin_role_rejects_non_admin():
    try:
        auth.require_admin_role(x_user_id="u1", x_user_role="read_only")
    except Exception:
        return
